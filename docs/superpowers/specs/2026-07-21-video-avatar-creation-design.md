...
- New video wizard (parallel to the existing photo one, same modal chrome):
  ...
  - On submit, `createVideoAvatarAction` runs; on success the modal closes and the new row appears with status `pending_consent`. **Implementation note (accepted deviation):** the consent link is surfaced via the row's persistent "Copy consent link" button (see below) rather than an in-modal banner, since `createVideoAvatarAction` doesn't return `consentUrl` to the client — this was reviewed and accepted as functionally equivalent, not an oversight.
